...
if %errorlevel% NEQ 0 (
echo:
echo Null 서비스가 실행중이지 않으므로 충돌할 수 있습니다.
echo:
echo:
...
set "nceline=echo: &echo ==== 오류 ==== &echo:"
set "eline=echo: &call :_color %Red% "==== 오류 ====" &echo:"
...
echo 이 스크립트는 윈도우 7/8/10/11 또는 해당 서버 버전을 지원합니다.
goto done2
)

for %%# in (powershell.exe) do @if "%%~$PATH:#"=="" (
%nceline%
echo 이 시스템에서 PowerShell 을 찾을 수 없습니다.
goto done2
)
...
%eline%
echo 스크립트가 임시 폴더에서 실행됨을 감지 했습니다.
echo 아마 압축 파일 내에서 실행 중인 것 같습니다.
echo:
echo 압축 파일의 압축을 풀고 실행해 주시길 바랍니다.
goto done2
)
)
...
echo:
echo PowerShell이 작동 되지 않음. 중단중...
echo PowerShell에 제한을 적용했다면 해당 변경 사항을 되돌리시길 바랍니다.
echo:
echo 도움말 %mas%idm-activation-script.html#Troubleshoot
goto done2
)
...
echo [0] 건너뛰기
echo:
call :_color %_Green% "키보드로 원하는 옵션을 입력해 주세요. [1,0] :"
choice /C:10 /N
if !errorlevel!==2 rem
...
echo:
echo:
echo:                 이 스크립트는 최신 IDM 버전과 호환되지 않습니다.  
...
echo:            ___________________________________________________
echo:         
call :_color2 %_White% "             " %_Green% "키보드로 원하는 옵션을 입력해 주세요. [1,2,3,4,5,0]"
choice /C:123450 /N
set _erl=%errorlevel%
...
echo %line%
echo:
echo      일부 유저에겐 활성화가 작동하지 않을 수 있으며, 가짜 시리얼로 인한 알림창이 뜰 수 있습니다.
echo:
call :_color2 %_White% "     " %_Green% "체험일수 동결을 추천합니다."
echo %line%
echo:
...
echo:
if not exist "%IDMan%" (
call :_color %Red% "IDM [Internet Download Manager]이 설치되지 않았습니다."
...
echo 정보 확인중 - [%regwinos% ^| %fullbuild% ^| %regarch% ^| IDM: %IDMver%]
...
echo:
echo %SystemRoot%\Temp 에 CLSID 백업 만드는 중...
...
call :_color %Green% "작업이 완료되었습니다."
echo:
call :_color %Gray% "만약 가짜 시리얼로 인한 알림이 뜰 경우, 활성화를 초기화하고 체험일수를 동결하시기 바랍니다."
) else (
call :_color %Green% "작업이 완료되었습니다."
echo:
call :_color %Gray% "만약 계속 등록하라는 팝업이 보인다면, IDM을 재설치 해 주세요."
)
...
if defined terminal (
call :_color %_Yellow% "0 을 눌러 돌아가기"
choice /c 0 /n
) else (
call :_color %_Yellow% "아무 키나 눌러 돌아가기"
pause %nul1%
)
goto MainMenu
...
echo:
echo 레지스트리 키를 생성하기 위해 몇 가지 다운로드를 실행하는 중. 잠시 기다려 주세요...
echo:
